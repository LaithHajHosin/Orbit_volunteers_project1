document.addEventListener("DOMContentLoaded", () => {
    ...
    fetch(proxyUrl, {
      ...
      .catch((err) => {
        ...
        document.querySelector("#blogs-tbody").innerHTML =
          '<tr><td colspan="5" style="color: var(--purple-3); text-align: center;">حدث خطأ في تحميل المقالات</td></tr>';
      });
  }
  ...
  function renderBlogs(blogs) {
    ...
    tbody.innerHTML = "";
    if (blogs.length === 0) {
      tbody.innerHTML = '<tr><td colspan="5" style="color: var(--purple-3); text-align: center;">لا توجد مقالات مضافة</td></tr>';
      return;
    }
    blogs.forEach((blog) => {
      ...
      tbody.innerHTML += `
        <tr>
          ...
          <td><span class="badge">${blog.category_name || "غير مصنف"}</span></td>
    ...
    fetch(proxyUrl, {
      ...
      .catch((err) => {
        ...
        document.querySelector("#categories-tbody").innerHTML =
          '<tr><td colspan="2" style="color: var(--purple-3); text-align: center;">حدث خطأ في تحميل التصنيفات</td></tr>';
      });
  }
  ...
  function renderCategories(categories) {
    ...
    if (categories.length === 0) {
      tbody.innerHTML = '<tr><td colspan="2" style="color: var(--purple-3); text-align: center;">لا توجد تصنيفات متوفرة</td></tr>';
      return;
    }
    ...
    fetch(PROXY_URL + encodeURIComponent(url), {
      ...
      .then((res) => {
        if (res.status === 401 || res.status === 403) {
          alert("غير مصرح! تأكدي من تسجيل الدخول بحساب الـ Admin لتنفيذ الإجراء.");
          throw new Error("Unauthorized access token status.");
        }
        if (!res.ok) throw new Error("HTTP error! status: " + res.status);
        return res.json();
      })
      .then(() => {
        alert(isEdit ? "تم تحديث المقالة برمجياً بنجاح!" : "تم إضافة المقالة بنجاح!");
        resetBlogForm();
        document.querySelector("#blog-form-container").classList.add("hidden");
        fetchBlogs();
      })
      .catch((err) => {
        console.error(err);
        alert("فشلت العملية أثناء معالجة المقال: " + err.message);
      });
  });
  ...
  document.querySelector("#category-form").addEventListener("submit", function(e) {
    ...
    fetch(PROXY_URL + encodeURIComponent(url), {
      ...
      .then((res) => {
        if (res.status === 401 || res.status === 403) {
          alert("غير مصرح! صلاحيات الـ Admin مطلوبة.");
      ...
      .then(() => {
        alert(isEdit ? "تم تعديل التصنيف بنجاح!" : "تم إضافة التصنيف الجديد بنجاح!");
        resetCategoryForm();
        document.querySelector("#category-form-container").classList.add("hidden");
        fetchCategories();
        fetchBlogs(); 
      ...
      .catch((err) => {
        console.error(err);
        alert("فشلت العملية أثناء معالجة التصنيفات: " + err.message);
      });
  });
